...
- `recall_top_5`: 상위 5개에서 찾은 필수 정답 조문 수 / 전체 필수 정답 조문 수
- `precision_top_5`: 상위 5개 중 필수 또는 보조 정답 조문인 조문 수 /
  실제 반환된 조문 수

`required_claims`는 조문 Recall 계산에 섞지 않고, 생성 답변이 꼭 말해야 할 내용을
...
LangSmith에는 질문을 Inputs, `reference_answer`를 Reference Outputs로 등록한다.
정답 조문과 평가 가능 여부 같은 채점 기준은 example의 `metadata.rubric`에 둔다.
덕분에 웹 표에서는 기대 답변을 바로 읽을 수 있고, evaluator는 rubric을 별도로
사용할 수 있다.

## 평가 함수가 반환하는 출력

`src/chatbot/evaluation.py`의 평가용 함수는 질문 하나를 받아 다음 세 가지를
돌려준다.
...
현재 24문항으로 LangChain v1 개발 기준선을 만들었다. 독립적인 최종 성능을 확인할
필요가 생기면 새 질문 10~12개를 별도 test split으로 만들고 설정 변경에는 사용하지
않는다.
