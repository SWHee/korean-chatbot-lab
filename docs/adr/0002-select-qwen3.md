...
- M2 통합 메모리에서 원본 모델과 긴 출력을 함께 실행하면 응답 지연과 메모리
  부담이 크다.
...
Python 3.13 환경에서 tokenizer와 chat template를 확인하고, 모델을 MPS에 적재한
뒤 한국어 prompt 생성을 검증했다. 이후 기본 backend는 응답 속도와 메모리 사용을
고려해 Ollama 양자화 모델로 전환했다(ADR 0003).
